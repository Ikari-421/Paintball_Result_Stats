...
describe('Field', () => {
    describe('Matchup', () => {
        it('should create a valid matchup', () => {
            const matchup = Matchup.create('matchup-1', 'team-a', 'team-b', 1);
        ...
        it('should throw error if id is empty', () => {
            expect(() => Matchup.create('', 'team-a', 'team-b', 1)).toThrow('Matchup ID cannot be empty');
        });

        it('should throw error if teamA is empty', () => {
            expect(() => Matchup.create('matchup-1', '', 'team-b', 1)).toThrow('Team A ID cannot be empty');
        });

        it('should throw error if teamB is empty', () => {
            expect(() => Matchup.create('matchup-1', 'team-a', '', 1)).toThrow('Team B ID cannot be empty');
        });

        it('should throw error if teamA and teamB are the same', () => {
            expect(() => Matchup.create('matchup-1', 'team-a', 'team-a', 1)).toThrow('Team A and Team B must be different');
        });

        it('should throw error if order is negative', () => {
            expect(() => Matchup.create('matchup-1', 'team-a', 'team-b', -1)).toThrow('Order cannot be negative');
        });
    });
    ...
    describe('addMatchup', () => {
        ...
        it('should preserve existing matchups when adding new one', () => {
            const matchup1 = Matchup.create('matchup-1', 'team-a', 'team-b', 1);
            const field = Field.create('field-1', 'Court Central', [matchup1]);
            
            const matchup2 = Matchup.create('matchup-2', 'team-c', 'team-d', 2);
